...
class CryBuy {
    ...
    async GetOrderBookGroup(d){
        ...
        for (const orderBook of orderBooks) {
            if(
                ...
                || !orderBook.asks
                || !orderBook.bids
                ||orderBook.asks[0].rate == 0.00000001 
                || orderBook.bids[0].rate < 0.00000022
            ) {
                return false
            }
        }
        ...
        */
        firstOrderBook = this.SetOrderBook(firstOrderBook, 'asks') // [{"Price": Number(firstOrderBook['asks'][0]['rate']),"Total": Number(firstOrderBook['asks'][0]['rate']) * Number(firstOrderBook['asks'][0]['amount'])}]
        secondOrderBook = this.SetOrderBook(secondOrderBook, 'bids') //[{"Price": Number(secondOrderBook['bids'][0]['rate']),"Total": Number(secondOrderBook['bids'][0]['rate']) * Number(secondOrderBook['bids'][0]['amount'])}]
        btcOrderBook = this.SetOrderBook(btcOrderBook, 'asks') //[{"Price": Number(btcOrderBook['asks'][0]['rate']),"Total": Number(btcOrderBook['asks'][0]['rate']) * Number(btcOrderBook['asks'][0]['amount'])}]
        if(d['type'] == 'alt'){
            thirdOrderBook = this.SetOrderBook(thirdOrderBook, 'asks') //[{"Price": Number(thirdOrderBook['asks'][0]['rate']),"Total": Number(thirdOrderBook['asks'][0]['rate']) * Number(thirdOrderBook['asks'][0]['amount'])}]
        }else{
            thirdOrderBook = this.SetOrderBook(thirdOrderBook, 'bids') // [{"Price": Number(thirdOrderBook['bids'][0]['rate']),"Total": Number(thirdOrderBook['bids'][0]['rate']) * Number(thirdOrderBook['bids'][0]['amount'])}]
        }

        return {firstOrderBook, secondOrderBook, thirdOrderBook, btcOrderBook}
    }

    SetOrderBook(orderBook, type){
        if(!orderBook) {
            return
        }
        return [{"Price": Number(orderBook[type][0]['rate']),"Total": Number(orderBook[type][0]['rate']) * Number(orderBook[type][0]['amount'])}]
    }
    ...
    async BuySellBasla(market){
        ...
        await this.ortak.mailDataEski.insertOne(market)
        //return
