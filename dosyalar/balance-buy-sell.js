...
class WsMongo {
    ...
    async cryWsBasla(){
        ...
        while(true){
            await this.RunForBalanceCoin()
            await this.ortak.sleep(30) // 30 saniye
        } 
    }

    async RunForBalanceCoin(){
        const openOrders = await this.ortak.GetFbData() 
        for (const order of openOrders) {
            const anaCoin = order.market.split('/')[0]
            const baseCoin = order.market.split('/')[1]
            this.AltcoinCheck(anaCoin, baseCoin)
        }
    }
    ...
    GetSellBases(baseCoin){
        switch (baseCoin) {
            case 'BTC': return {first: 'LTC', second: 'DOGE'}
            case 'LTC': return {first: 'BTC', second: 'DOGE'}
            case 'DOGE': return {first: 'BTC', second: 'LTC'}
        }
    }

    AltcoinCheck(anaCoin, baseCoin){
        ...
        const {first, second} = this.GetSellBases(baseCoin)

        const sonuc1 = this.MarketeGir(anaCoin, baseCoin, first)
        if(sonuc1) uygunMarkets.push(sonuc1)       

        const sonuc2 = this.MarketeGir(anaCoin, baseCoin, second)
        if(sonuc2) uygunMarkets.push(sonuc2)
