...
class Ortak {
    async LoadVeriables(type){
        ...
        this.key = 'hza6xX0vDuLRVkUszWFjiC7P' // Testnet
        this.secret = 'WYMgkaRPgxNCdCZpkNjN-lH0qrIh8UbtVawdDxbMxRcl8lP6'
        this.bitmexTest = true
        this.bitmexUrl = 'https://testnet.bitmex.com'

        this.ccx = new MhtCcxt(this.key, this.secret, this.site, null)
        if(this.bitmexTest) this.ccx.exchange.urls['api'] = this.ccx.exchange.urls['test']
...
*/
    ...
    async BitmexPositions(){
        ...
        var signature = crypto.createHmac('sha256', this.secret).update(verb + path + expires).digest('hex');
        ...
        return await rp(requestOptions) //(error, response, body) => {
    }

    async BitmexCalcelAllOrders(){
        var verb = 'DELETE',
